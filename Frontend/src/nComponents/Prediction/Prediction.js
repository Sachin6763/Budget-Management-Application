...
function Prediction({ Username }) {
  ...
  function predictNextDayExpense(expenseData) {
    ...
    // Calculate average daily expenses for the current month
  ...
  function predictNextDayIncome(incomeData) {
    ...
    // Calculate average daily incomes for the current month
    ...
    const nextDay = today.getDate() + 1;
    const predictedIncome = averageDailyIncome.toFixed(2); // Round to 2 decimal places
  ...
  return (
    <div className="prediction">
      <div className="prediction-sidebar">
        ...
        {/* <button
          onClick={() => handleClick("prediction-savings")}
          className={`prediction-button ${
            activeComponent === "prediction-savings" ? "active" : ""
          }`}
        >
